...
class Preprocess:
    ...
    def __post_init__(self) -> None:
        """
        Performs exception handling to catch invalid directory.
    ...
    def __load_dataset(
        self, labels: List, load_format: str = ".npy"
    ) -> Tuple[np.ndarray]:
        """
        ...
        for index, label in enumerate(self.labels[1:]):
            ...
            labels = np.append(labels, np.full(x.shape[0], fill_value=(index + 1)))

        return data, labels

    def preprocess_dataset(self, labels: List, test_split_percent: float) -> Dataset:
        """
        ...
        x_train, x_test, y_train, y_test = train_test_split(
            X, y, test_size=test_split_percent, random_state=42, shuffle=True
        )

        for data in (x_train, x_test, y_train, y_test):
            if data is None:
                raise ValueError(
                    f"{data} is null. Please check and preprocess again!!!"
                )

        return Dataset(
            x_train,
            to_categorical(y_train, num_classes=len(labels)),
            x_test,
            to_categorical(y_test, num_classes=len(labels)),
        )

    def dump_audio_files(
        self,
        audio_files_dir: str,
        labels: List,
        n_mfcc: int,
        ...
        for label in labels:
            ...
            audio_files = [
                audio_files_dir + label + "/" + audio_file
                for audio_file in os.listdir(audio_files_dir + "/" + label)
            ]
            for audioFile in tqdm(audio_files):
                mfcc_features = convert_audio_to_mfcc(
                    audioFile, n_mfcc, mfcc_length, sampling_rate
                )
...
def convert_audio_to_mfcc(
    audio_file_path: str, n_mfcc: int, mfcc_length: int, sampling_rate: int
) -> np.ndarray:
    """
    ...
    mfcc_features: np.ndarray = librosa.feature.mfcc(
        audio, n_mfcc=n_mfcc, sr=sampling_rate
    )
    if mfcc_length > mfcc_features.shape[1]:
        padding_width = mfcc_length - mfcc_features.shape[1]
        mfcc_features = np.pad(
            mfcc_features, pad_width=((0, 0), (0, padding_width)), mode="constant"
        )
    else:
