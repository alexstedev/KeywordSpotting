"""Script for inferencing with new or test data."""
...
from src.exception_handler import DirectoryError, NotFoundError, ValueError
...
class KeywordSpotter:
    ...
    def predict(self) -> Tuple[str, float]:
        ...
        try:
            ...
            audio_mfcc: np.ndarray = data.convert_audio_to_mfcc(
                self.audio_file, self.n_mfcc, self.mfcc_length, self.sampling_rate
            )
            ...
            label_probability: float = max(
                [
                    round(value, 4)
                    for value in list(
                        dict(enumerate(model_output.flatten(), 1)).values()
                    )
                ]
            )
            ...
            raise NotFoundError(
                f"Cannot infer from model. Please check the paths and try it again !!! {exc}"
            ) from exc


@hydra.main(config_path="config_dir", config_name="config")
def main(cfg: KWSConfig) -> None:
    inference_ = KeywordSpotter(
    ...
    print(
        f"Predicted keyword: {predicted_keyword} \n Keyword probability: {label_probability}"
    )
